...
from tkinter import filedialog, messagebox, Button, Entry, Frame, Canvas, Scrollbar, ttk, Label, Tk, LEFT, RIGHT, BOTH, Y
import threading
import styles

class PDFMergerApp:
    def __init__(self, root, pdf_entries, enable_organize_callback, go_back_callback):
        self.root = root
        self.pdf_entries = pdf_entries  # Referência para a lista de PDFs no MainApp
        self.enable_organize_callback = enable_organize_callback  # Callback para habilitar botão
    ...
    def create_widgets(self):
        ...
        self.add_button = Button(self.frame, text="+", command=self.browse_files, font=("Arial", 14), width=5, height=2)
        ...
        self.back_button.grid(row=2, column=3, padx=5, pady=5, sticky='w')
    ...
    def browse_files(self):
        ...
        if file_paths:
            ...
            self.enable_organize_callback()  # Habilitar botão de organizar PDFs
...
if __name__ == "__main__":
    def enable_organize_callback():
        pass  # Callback de habilitação

    def go_back_callback():
        pass  # Callback de retorno

    root = Tk()
    app = PDFMergerApp(root, [], enable_organize_callback, go_back_callback)
    root.mainloop()
